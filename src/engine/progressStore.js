...
export class ProgressStore {
    ...
    contentPackId() {
        if (this._memory)
            return this._memory.contentPackId;
        return this._settings.get_string('content-pack-id') || 'core';
    }

    setContentPackId(packId) {
        if (this._memory) {
            this._memory.contentPackId = packId;
            return;
        }
        this._settings.set_string('content-pack-id', packId);
    }

    isFirstRunComplete() {
        if (this._memory)
            return this._memory.firstRunComplete;
        return this._settings.get_boolean('first-run-complete');
    }

    markFirstRunComplete() {
        if (this._memory) {
            this._memory.firstRunComplete = true;
            return;
        }
        this._settings.set_boolean('first-run-complete', true);
    }
}
